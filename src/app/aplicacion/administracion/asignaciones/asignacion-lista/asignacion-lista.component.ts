...
@Component({
  selector: 'app-asignacion-lista',
  templateUrl: './asignacion-lista.component.html',
  styleUrl: './asignacion-lista.component.scss',
  standalone: true,
  imports: [
...
export class AsignacionListaComponent {
  ...
  ngOnInit(): void {
    this.titleService.setTitle('Asignaciones');
  ...
  obtenerConductores(): void {
    ...
    this.conductorServicio.obtenerTodosActivos().then(res => {
      this.listaConductores = [
        { id: 'TODOS', dato: 'TODOS' },
        ...res.map((res: any) => {
          res.dato = res.apellidos + ' - ' + res.nombres;
          return res;
        })
      ];
      // this.listaVehiculos = res;
  ...
  obtenerConsulta(): void {
    this.cargando.show();
    this.asignacionServicio.obtenerConsulta(this.buscadorFormGroup.getRawValue()).then((respuesta: any) => {
  ...
  eliminar(fila: any) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.cargando.show();
        this.asignacionServicio.editar(fila.id, { activo: false, usuarioElimina: this.usuario.email }).then(result => {
          this.cargando.hide();
