...
@Component({
  ...
  styleUrl: './reporte5.component.scss',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
...
export class Reporte5Component {
  ...
  transformarDatos(lista: any[]): any[] {
    ...
    for (const item of lista) {
      ...
      const min = this.getMinimo(item);
      const { estado, faltante } = this.calcularResultado(item);

      // Texto para la columna Resultado (una sola celda)
      let textoResultado = '';
      if (estado === 'SIN_MINIMO') textoResultado = 'SIN MÍNIMO';
      else if (estado === 'PEDIR') textoResultado = `PEDIR (faltan ${faltante})`;
      else textoResultado = 'TIENE';
      ...
      const base = {
        ...
        Saldo: saldo,
        // opcionalmente expórtalo también como número:
        // Minimo: min ?? '',
        // Faltante: estado === 'PEDIR' ? faltante : 0,
      };
  ...
  // Helpers robustos ante null/undefined/'' y strings numéricos
  esMinimoDefinido(item: any): boolean {
    const m = item?.minimo;
    return m !== null && m !== undefined && m !== '' && !isNaN(Number(m));
  }

  getMinimo(item: any): number | null {
    return this.esMinimoDefinido(item) ? Number(item.minimo) : null;
  }

  estadoInventario(item: any): 'SIN_MINIMO' | 'PEDIR' | 'TIENE' {
    const min = this.getMinimo(item);
    if (min === null) return 'SIN_MINIMO';
    return item.cantidadSaldoTotal < min ? 'PEDIR' : 'TIENE';
  }

  // Opcional: cuántas unidades faltan para llegar al mínimo
  faltante(item: any): number {
    const min = this.getMinimo(item);
    if (min === null) return 0;
    return Math.max(0, min - Number(item.cantidadSaldoTotal || 0));
  }

  private esNumeroValido(v: any): boolean {
    return v !== null && v !== undefined && v !== '' && !isNaN(Number(v));
  }

  private calcularResultado(item: any): { estado: 'SIN_MINIMO' | 'PEDIR' | 'TIENE', faltante: number } {
    const min = this.getMinimo(item);
    const saldo = Number(item?.cantidadSaldoTotal ?? 0);

    if (min === null) return { estado: 'SIN_MINIMO', faltante: 0 };

    if (saldo < min) {
      return { estado: 'PEDIR', faltante: Math.max(0, min - saldo) };
    }
    return { estado: 'TIENE', faltante: 0 };
  }

  // A EXCEL
  exportarJsonAExcel(): void {
    this.excelServicio.exportarAExcel(this.listaExpotacion, 'SaldoMinimo-');
  }
