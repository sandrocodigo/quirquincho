...
export class ProgramacionFormComponent {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<ProgramacionFormComponent>,
    ...
    this.authServicio.user$.subscribe((user) => {
      if (user) {
        ...
        if (data.nuevo) {
          ...
          // FORM EDITAR
          this.cargando.show();
          this.pServicio.obtenerPorId(this.id).then((respuesta: any) => {
            ...
            this.registroFormGroup = this.fb.group({
              ...
              mantenimientoId: [respuesta.mantenimientoId, [Validators.required]],
              mantenimientoTipo: [respuesta.mantenimientoTipo, [Validators.required]],
              mantenimientoDescripcion: [respuesta.mantenimientoDescripcion, [Validators.required]],
              mantenimientoFrecuencia: [respuesta.mantenimientoFrecuencia, [Validators.required]],
              mantenimientoKilometraje: [respuesta.mantenimientoKilometraje, [Validators.required]],

              kilometraje: [respuesta.kilometraje, [Validators.required]],
  ...
  obtenerMantenimientos(): void {
    ...
    this.mantenimientoService.obtenerTodos().then((respuesta: any[]) => {
      ...
      this.listaMantenimientos = respuesta
        .sort((a, b) => Number(a.id) - Number(b.id)) // Orden ascendente por ID numérico
        .map(item => ({
          ...item,
          dato: `${item.id} : ${item.tipo} - ${item.descripcion}`
        }));
