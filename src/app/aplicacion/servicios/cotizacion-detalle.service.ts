...
export class CotizacionDetalleService {
  ...
  private url = 'cotizaciones-detalles';
  ...
  // OBTENER POR COTIZACION
  async obtenerPorCotizacion(idCotizacion: any): Promise<any[]> {
    let q = query(
      collection(this.firestore, `${this.url}`) as CollectionReference<any>,
      where('cotizacionId', '==', idCotizacion),
      orderBy('productoTipo'),
