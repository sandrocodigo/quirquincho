      ...
      switchMap((user) => {
        ...
        return from(this.usuarioService.obtenerPorId(user.email)).pipe(
          map((userData: any) => {
            ...
            // Verificar si el usuario tiene acceso a la ruta actual o sus subrutas
            const tieneAcceso = userData.accesos.some((acceso: any) => rutaActual.startsWith(acceso.link));
